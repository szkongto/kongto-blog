"""Generate missing P0 product pages from keyword map."""
import os, sys
...
PRODUCTS = [
    {
        'slug': 'heidenhain-bc110-lcd-upgrade',
        'title': 'Heidenhain BC110 LCD Replacement | 9" Mono CRT to LCD | Kongto',
        'desc': 'Heidenhain BC110 LCD replacement — 9-inch monochrome CRT to LCD upgrade for TNC 310/320 controls. Plug-and-play, no rewiring. 2-year warranty, ships today.',
        'h1': 'Heidenhain BC110 LCD Replacement Display',
        'brand': 'Heidenhain',
        'brand_page': 'Heidenhain.html',
        'price': '$355',
        'sku': 'KONGTO-BC110-LCD',
        'mpn': 'BC110-LCD',
        'compat': 'Heidenhain TNC 310, TNC 320',
        'signal': '15-pin D-Sub (RGBHV) — CGA-compatible',
        'size': '9-inch',
    },
    {
        'slug': 'heidenhain-be411-lcd-upgrade',
        'title': 'Heidenhain BE411 LCD Replacement | 9" Mono CRT to LCD | Kongto',
        'desc': 'Heidenhain BE411 LCD replacement — 9-inch monochrome CRT to LCD for TNC 410/415 controls. Plug-and-play, no parameter changes. 2-year warranty, ships today.',
        'h1': 'Heidenhain BE411 LCD Replacement Display',
        'brand': 'Heidenhain',
        'brand_page': 'Heidenhain.html',
        'price': '$355',
        'sku': 'KONGTO-BE411-LCD',
        'mpn': 'BE411-LCD',
        'compat': 'Heidenhain TNC 410, TNC 415',
        'signal': '15-pin D-Sub (RGBHV) — CGA-compatible',
        'size': '9-inch',
    },
    {
        'slug': 'fanuc-om-d-display-lcd-upgrade',
        'title': 'FANUC OM-D Display Replacement | CRT to LCD | Kongto',
        'desc': 'FANUC OM-D display replacement — 9-inch CRT to LCD for FANUC OM-D CNC controls. Plug-and-play, same Honda 20-pin connector. 2-year warranty, ships today.',
        'h1': 'FANUC OM-D Display Replacement',
        'brand': 'FANUC',
        'brand_page': 'FANUC.html',
        'price': '$255',
        'sku': 'KONGTO-OMD-LCD',
        'mpn': 'OM-D-LCD',
        'compat': 'FANUC OM-D, FANUC 0-Mate',
        'signal': 'Honda MR-20M 20-pin — TTL RGB',
        'size': '9-inch',
    },
    {
        'slug': 'siemens-6fc3998-7fa20-lcd-upgrade',
        'title': 'Siemens 6FC3998-7FA20 LCD Upgrade | SINUMERIK 850/880 | Kongto',
        'desc': 'Siemens 6FC3998-7FA20 LCD upgrade — 8-inch replacement for SINUMERIK 850/880 CRT. Plug-and-play, no rewiring. 2-year warranty, ships today.',
        'h1': 'Siemens 6FC3998-7FA20 LCD Upgrade Display',
        'brand': 'Siemens',
        'brand_page': 'Siemens.html',
        'price': '$399',
        'sku': 'KONGTO-6FC3998-LCD',
        'mpn': '6FC3998-7FA20-LCD',
        'compat': 'Siemens SINUMERIK 850, 880',
        'signal': 'DB-25 / 25-pin D-Sub',
        'size': '8-inch',
    },
]

def sub(t, **kw):
    """Safe string substitution - replaces {key} with value, ignores CSS braces"""
    for k, v in kw.items():
        t = t.replace('{' + k + '}', str(v))
    return t
...
for p in PRODUCTS:
    ...
    html = sub(HEAD + TAIL, **p)
    fp = os.path.join(SITE, 'products', f"{p['slug']}.html")
    with open(fp, 'w', encoding='utf-8') as f:
        f.write(html)
    print(f'CREATED: products/{p["slug"]}.html ({os.path.getsize(fp)} bytes)')

print(f'\nDone. {len(PRODUCTS)} pages created.')
