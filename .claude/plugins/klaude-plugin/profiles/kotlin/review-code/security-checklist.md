...
- CORS & Headers
- Runtime Risks (unbounded ops, coroutine leaks, resource exhaustion)
- Cryptography
...
- **XSS**: Unsafe HTML injection, unescaped output in Ktor HTML DSL/Thymeleaf/Freemarker templates, raw string interpolation in HTTP responses
- **Injection**: SQL injection via string templates instead of parameterized queries; command injection via `Runtime.exec` or `ProcessBuilder` with user input; JNDI injection via unsanitized lookup names
...
- Unpinned dependencies allowing malicious updates (version ranges in `build.gradle.kts`)
- Dependency confusion (private package name collision in Maven Central)
...
- Blocking operations inside coroutines (blocking I/O on `Dispatchers.Main`/`Default` instead of `Dispatchers.IO`)
- **Coroutine scope leaking**: `GlobalScope.launch` or unmanaged `CoroutineScope` without lifecycle binding — coroutines run indefinitely
- Resource exhaustion (file handles, connections, coroutine leaks without structured concurrency)
- **`runCatching` swallowing `CancellationException`**: `runCatching { suspendingCall() }` catches `CancellationException`, breaking structured concurrency cancellation
...
- Using `==` for secret/token comparison instead of `MessageDigest.isEqual` (leaks timing information)
...
### Shared State Access
- Multiple coroutines/threads accessing shared variables without `Mutex`, `synchronized`, or `AtomicReference`
- Global state or singletons modified concurrently
- Lazy initialization without proper locking (`lazy` with incorrect thread-safety mode)
- Non-thread-safe collections (`HashMap`, `mutableListOf`) used in concurrent context
...
- Weak validation before persistence (unsafe casts via `as`)
