...
for file in "${FILES[@]}"; do
    if [ -f "$file" ]; then
        echo "修复: $file"
        # 使用Python来更可靠地处理多行字符串替换
        python3 -c "
import re
with open('$file', 'r', encoding='utf-8') as f:
    content = f.read()

# 修复模式1: modal.innerHTML = '...<div class=\"wechat-modal-content\">\n...'
# 把多行的wechat modal innerHTML内容合并成一行
pattern = r\"(modal\.innerHTML = '<div class=\\\"wechat-modal-content\\\"'>)\s*\n\s*(<h3[^>]*>)\"
replacement = r\"\1\2\"
new_content = re.sub(pattern, replacement, content)

# 继续合并后续的换行和标签
# 合并 </div>' 到同一行
pattern2 = r\"(onclick=\\\"this\.parentElement\.parentElement\.remove\(\)\\\">关闭</button></div>'\"
if 'onclick=\"this.parentElement.parentElement.remove()\">关闭</button></div>' in new_content:
    # 已经是正确的格式，检查是否有换行在</div>'之前
    pass

# 合并 h3 标签后面的内容（如果有换行）
pattern3 = r\"(modal\.innerHTML = '<div class=\\\"wechat-modal-content\\\"[^<]*<h3[^>]*>[^<]*</h3>)\s*\n\s*(<img)\"
replacement3 = r\"\1\2\"
new_content = re.sub(pattern3, replacement3, new_content)

# 合并 img 标签后面的内容
pattern4 = r\"(modal\.innerHTML = '<div class=\\\"wechat-modal-content\\\"[^<]*<img[^>]+>)\s*\n\s*(<p)\"
replacement4 = r\"\1\2\"
new_content = re.sub(pattern4, replacement4, new_content)

# 合并 p 标签后面的内容
pattern5 = r\"(modal\.innerHTML = '<div class=\\\"wechat-modal-content\\\"[^<]*<p[^>]*>[^<]*</p>)\s*\n\s*(<button)\"
replacement5 = r\"\1\2\"
